...
class JoyMapper(object):
    def __init__(self):
        ...
        self.pub_speed_sate = rospy.Publisher("~speed_state",JoystickSpeedStamped,queue_size=1)
    ...
    def publishControl(self):
        ...
        #rospy.loginfo("self.v_gain = %f",self.v_gain)
...
# logitek = 8, left joy = 9, right joy = 10
    ...
    def processButtons(self, joy_msg):
        ...
        elif (joy_msg.buttons[0] == 1): # push 'A' => speed up
            rospy.loginfo("[%s] start speed up",self.node_name)
            speed_state_msg = JoystickSpeedStamped()
            speed_state_msg.header.stamp = self.joy.header.stamp
            speed_state_msg.speed_state = 1 # 1 =>speed up
            self.pub_speed_sate.publish(speed_state_msg)

        elif (joy_msg.buttons[1] == 1): # push 'B' => speed down  
            rospy.loginfo("[%s] start speed down",self.node_name)
            speed_state_msg = JoystickSpeedStamped()
            speed_state_msg.header.stamp = self.joy.header.stamp
            speed_state_msg.speed_state = 2 # 0 =>speed up
            self.pub_speed_sate.publish(speed_state_msg)
    ...
    def getFilePath(self, name):
        ...
        return rospack.get_path('duckietown') + '/config/baseline/joy_mapper/joy_mapper_node/' + name + ".yaml"
